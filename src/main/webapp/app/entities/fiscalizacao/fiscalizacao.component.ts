import { Component, OnInit, OnDestroy } from '@angular/core';
import { HttpErrorResponse, HttpResponse } from '@angular/common/http';
import { Subscription } from 'rxjs';
import { JhiEventManager, JhiAlertService } from 'ng-jhipster';

import { IFiscalizacao } from 'app/shared/model/fiscalizacao.model';
import { Principal } from 'app/core';
import { FiscalizacaoService } from './fiscalizacao.service';

@Component({
    selector: 'jhi-fiscalizacao',
    templateUrl: './fiscalizacao.component.html'
})
export class FiscalizacaoComponent implements OnInit, OnDestroy {
    fiscalizacaos: IFiscalizacao[];
    currentAccount: any;
    eventSubscriber: Subscription;

    constructor(
        private fiscalizacaoService: FiscalizacaoService,
        private jhiAlertService: JhiAlertService,
        private eventManager: JhiEventManager,
        private principal: Principal
    ) {}

    loadAll() {
        this.fiscalizacaoService.query().subscribe(
            (res: HttpResponse<IFiscalizacao[]>) => {
                this.fiscalizacaos = res.body;
            },
            (res: HttpErrorResponse) => this.onError(res.message)
        );
    }

    ngOnInit() {
        this.loadAll();
        this.principal.identity().then(account => {
            this.currentAccount = account;
        });
        this.registerChangeInFiscalizacaos();
    }

    ngOnDestroy() {
        this.eventManager.destroy(this.eventSubscriber);
    }

    trackId(index: number, item: IFiscalizacao) {
        return item.id;
    }

    registerChangeInFiscalizacaos() {
        this.eventSubscriber = this.eventManager.subscribe('fiscalizacaoListModification', response => this.loadAll());
    }

    private onError(errorMessage: string) {
        this.jhiAlertService.error(errorMessage, null, null);
    }
}
